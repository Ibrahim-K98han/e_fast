import 'dart:convert';
import 'package:e_fast/Models/login_models.dart';
import 'package:e_fast/Models/refresh_token_model.dart';
import 'package:e_fast/Models/register_model.dart';
import 'package:e_fast/controllers/global_controller.dart';
import 'package:e_fast/screen/auth/login_screen.dart';
import 'package:e_fast/screen/auth/verify_otp.dart';
import 'package:e_fast/screen/main/main_screen.dart';
import 'package:e_fast/services/api_list.dart';
import 'package:e_fast/services/service.dart';
import 'package:e_fast/services/user_service.dart';
import 'package:e_fast/services/validators.dart';
...
class AuthController extends GetxController {
      ...
      server.postRequest(endPoint: APIList.login, body: jsonBody).then((
        ...
        if (response != null && response.statusCode == 200) {
          ...
          userService.saveString(
            key: 'user-id',
            value: loginData.data!.user?.id.toString(),
          );
          userService.saveString(
            key: 'email',
            value: loginData.data!.user!.email.toString(),
          );
          userService.saveString(
            key: 'image',
            value: loginData.data!.user!.image.toString(),
          );
          userService.saveString(
            key: 'name',
            value: loginData.data!.user!.name.toString(),
          );
          userService.saveString(
            key: 'phone',
            value: loginData.data!.user!.phone.toString(),
          );
  ...
  otpVerification(mobile) async {
    ...
    Map body = {
      'otp':
          '${otp1.text.toString() + otp2.text.toString() + otp3.text.toString() + otp4.text.toString() + otp5.text.toString()}',
      'mobile': mobile,
    };
    String jsonBody = json.encode(body);
    print(jsonBody);
    server.postRequest(endPoint: APIList.verifyOtp, body: jsonBody).then((
      response,
    ) {
      if (response != null && response.statusCode == 200) {
        final jsonResponse = json.decode(response.body);
